...
const annotationTemplates = {
  ...
  'oversplit': {
    label: 'Oversplitting',
    prefill: 'This word does not need to be split up so much. It would be better if it was synced as a whole word.'
  },
...
function parseTTML(xmlDoc) {
  const songwriters = Array.from(xmlDoc.querySelectorAll('songwriter')).map(s => s.textContent.trim()).filter(Boolean);
  const hasSongwriters = songwriters.length > 0;
  const ttTag = xmlDoc.querySelector('tt');
  const timing = ttTag ? (ttTag.getAttribute('itunes:timing') || ttTag.getAttribute('itunes\\:timing') || ttTag.getAttribute('timing') || '') : '';
  ...
  for (const p of paragraphs) {
    ...
    if (mainText) {
      cues.push({ text: mainText, timestamp, isRight: agent === 'v2', isAdlib: false });
    }

    if (adlibText) {
      cues.push({ text: adlibText, timestamp, isRight: agent === 'v2', isAdlib: true });
    }
  }
...
async function loadTTMLFromContent(xmlText) {
  ...
  try {
    ...
    if (parsererror) throw new Error('Invalid XML/TTML: ' + parsererror.textContent);
    ...
    cues.forEach((cue, index) => {
      ...
      cueEl.className = `cue ${cue.isRight ? 'line-right' : ''} ${cue.isAdlib ? 'adlib' : ''}`.trim();
...
const annotationViewerCloseBtn = document.getElementById('annotationViewerCloseBtn');
...
function shouldOverrideText(reason) {
  return ['sync-too-early', 'sync-too-late', 'generic-resync', 'adlib-mainline', 'missing-words', 'misheard-words', 'inaccurate-transcription', 'other'].includes(reason);
}
...
function insertSelectedTextSelectionByLine() {
  ...
  const linesInRange = Array.from(container.querySelectorAll('.line-main')).filter(line => {
...
function updateLineHighlightState() {
  document.querySelectorAll('.line-main').forEach(line => {
    const lineId = line.dataset.lineId;
    const hasFallback = annotations.some(a => a.lineIds.includes(lineId) && (!a.highlightElements || a.highlightElements.length === 0));
    if (hasFallback) {
...
annotationSaveBtn.addEventListener('click', () => {
  ...
  const reasonText = annotationReason.options[annotationReason.selectedIndex]?.text || reasonKey;
  ...
  const highlightEls = applyAnnotationRangeHighlight(currentSelectionInfo.range, currentSelectionInfo.lines);
...
annotationEditBtn.addEventListener('click', () => {
  ...
  const key = Object.keys(annotationTemplates).find(k => annotationTemplates[k].label === existing.reason) || 'other';
...
exportBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    setTimeout(() => { exportBtn.textContent = 'Export annotations'; }, 1500);
  } catch (error) {
